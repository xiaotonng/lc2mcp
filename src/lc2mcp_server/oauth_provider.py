...
class CustomOAuthProvider(OAuthProvider):
    ...
    async def exchange_authorization_code(
        ...
        async with get_db_session() as db:
            ...
            await db.execute(delete(OAuthCode).where(OAuthCode.code == authorization_code.code))
    ...
    async def revoke_token(self, token) -> None:
        ...
        async with get_db_session() as db:
            ...
            if isinstance(token, AccessToken):
                await db.execute(
                    delete(OAuthTokenModel).where(OAuthTokenModel.access_token == token.token)
                )
            elif isinstance(token, RefreshToken):
                await db.execute(
                    delete(OAuthTokenModel).where(OAuthTokenModel.refresh_token == token.token)
                )
